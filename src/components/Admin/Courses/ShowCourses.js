...
const ShowCourses = () => {
    ...
    const check = () => {
        axios.get(`http://localhost:1516/get/courses`, {
            params: {
            ...
            <table className="table-auto w-full">
                <thead>
                    <tr>
                        ...
                        <button className="btn-primary" onClick={check}>{isElementVisible ? "Hide Table" : "Show Table"}</button>
                    </tr>
                </thead>
                {isElementVisible && (
                    <tbody>
                        {courses.map((item, index) => (
                            <tr key={index} className={`${index % 2 === 0 ? "bg-gray-100" : "bg-white"}`}>
                                ...
                                <td className="border px-4 py-2">{item.courseDuration + " Years"}</td>
